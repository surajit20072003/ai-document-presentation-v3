...
**Change Log**: Added gesture controls (pinch-zoom confirmed), player visual direction confirmed from Trig player (80% baseline). 2026-03-08: Full interactivity system defined — taxonomy, params contract, Director rules, code generator patterns, validator rules, JSON schema.
...
- `understanding_quiz` — LLM-generated quiz section
- `renderer: "threejs"` — new valid renderer value
- `image_source` — when source contains image, Director receives and analyses it
...
STEP 2 — Source image present:
    Director analyses image → threejs_spec describes it animated and interactive
```
...
| Understanding quiz | N/A | Validated if `has_understanding_quiz: true` |
...
6. **LLM decides understanding quiz** — Only when content has testable concepts.
...
PHASE B — Three.js pipeline + interactivity ← CURRENT
  B1: threejs_code_generator.py (use threejs_system_prompt_v2.txt + fill_prompt_v2.py)
  B2: director_v3_partition_prompt.txt (new schema + interactivity taxonomy)
  ...
  B5: Interaction hint UI (e.g. "Drag the point ↑" fades in at beat start)

PHASE C — Quiz, Validation, Polish
  C1: Pre-baked quiz branching (3 avatar clips per question)
  C2: Understanding quiz generation
  C3: Update v25_validator.py (new interaction rules)
  C4: v3.0_Director_Bible.md published
```