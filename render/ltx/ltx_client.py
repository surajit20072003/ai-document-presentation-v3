...
LTX_BASE_URL = os.environ.get("LTX_BASE_URL", "http://69.197.145.4:8000")
...
class LtxClient:
    ...
    def generate_video(
        ...
        if duration is not None:
            ...
            print(
                f"[LTX] Dynamic duration requested: {duration}s -> {num_frames} frames at {frame_rate}fps"
            )
            ...
            try:
                ...
                if status == "completed":
                    ...
                    if not url and "output" in data:
                        url = data["output"].get("url") or data["output"].get(
                            "video_url"
                        )

                    if not url:
                        print(
                            f"[LTX] Completed but URL not apparent in keys: {list(data.keys())}"
                        )
                        # Fallback: maybe it's constructed from ID? Or check /api/videos/list filtered?
                        # For now, raise specific error to debug.
                        raise Exception(
                            f"Completed but no URL found. Data keys: {list(data.keys())}"
                        )
