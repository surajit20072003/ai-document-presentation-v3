...
from core.wan_prompt_validator import (
    hard_fail_on_short_prompts,
    WanPromptHardFailError,
    truncate_video_prompts,
    truncate_wan_prompt,
    expand_video_prompts,
)
...
def render_ltx_video(
    ...
    video_prompts = topic.get("video_prompts", []) or explanation_plan.get(
        "video_prompts", []
    )
        ...
        if use_precompiled:
            ...
            prompt = (
                ...
                if isinstance(p_obj, str)
                    ...
                    p_obj.get("prompt")
                    or p_obj.get("video_prompt")
                    or p_obj.get("wan_prompt")
                    or p_obj.get("description")  # V3 recap uses 'description' key
                    or ""
                )
            )
            duration = (
                ...
                if isinstance(p_obj, str)
                    ...
                    p_obj.get("duration_seconds") or p_obj.get("duration_hint") or 5
                )
            )
        else:
...
def _render_ltx_recap(
    topic_id, topic_title, recap_scenes, output_dir, dry_run, skip_wan, trace_output_dir
):
    """Render recap scenes."""
    video_paths = []
    client = LtxClient() if not dry_run and not skip_wan else None
    ...
    for i, scene in enumerate(recap_scenes):
        ...
        duration = int(
            scene.get("duration_seconds") or scene.get("duration_hint") or 15
        )
