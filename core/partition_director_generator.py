...
class PartitionDirectorGenerator:
    ...
    def __init__(self, config: Optional[GeneratorConfig] = None,
        ...
        self.content_prompt_file = content_prompt_file or "core/prompts/director_partition_prompt.txt"
        self.global_prompt_file = global_prompt_file or "core/prompts/director_global_prompt.txt"
        ...
        try:
            ...
            while retries < max_retries:
                try:
                    ...
                    if not sections_result:
                        ...
                        for sec in sections_result:
                            if not sec.get("renderer"):
                                errors.append(f"Section '{sec.get('title', '?')}' is missing 'renderer' field.")
                            if not sec.get("narration"):
                                errors.append(f"Section '{sec.get('title', '?')}' is missing 'narration' field.")
                    ...
                    logger.warning(f"Worker {index} Structural Check Failed (Attempt {retries+1}/{max_retries}): {errors}")
                    error_msg = "\n- ".join(errors)
                    current_prompt += (
                        f"\n\n[SYSTEM: PREVIOUS ATTEMPT REJECTED]\n"
                        f"Your previous JSON output was invalid for the following reasons:\n- {error_msg}\n"
