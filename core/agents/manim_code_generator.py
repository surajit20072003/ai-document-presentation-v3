...
class ManimCodeGenerator:
        ...
        if self._system_prompt is None:
            ...
            if not os.path.exists(system_path):
                system_path = os.path.join(
                    self.prompts_dir, "manim_system_prompt_v25.txt"
                )
        ...
        if self._user_template is None:
            ...
            if not os.path.exists(template_path):
                template_path = os.path.join(
                    self.prompts_dir, "manim_user_prompt_v25.txt"
                )
    ...
    def _build_user_prompt(self, section_data: Dict[str, Any]) -> str:
        ...
        narration_segments_text = self._format_segments(
            section_data.get("narration_segments", [])
        )

        # FIX 2: Soften the Director's spec to allow for Simplification
        raw_visual = section_data.get("manim_spec") or section_data.get(
            "visual_description", "Create appropriate visualization"
        )
        ...
        total_duration = sum(
            ...
            for seg in section_data.get("narration_segments", [])
        )
...
"""
    ...
    def _format_segments(self, segments: List[Dict]) -> str:
        ...
        for i, seg in enumerate(segments, 1):
            ...
            lines.append(
                f"Segment {i} ({current_time:.1f}s - {end_time:.1f}s, duration {duration:.1f}s):"
            )
    ...
    def generate(self, section_data: Dict[str, Any]) -> Tuple[str, List[str]]:
        ...
        logger.info(
            f"[MANIM GEN] Starting generation for section: {section_data.get('section_title', 'Unknown')}"
        )
        logger.debug(
            f"[MANIM GEN] Narration segments: {len(section_data.get('narration_segments', []))}"
        )
        logger.debug(
            f"[MANIM GEN] Visual description: {section_data.get('visual_description', 'None')[:100]}"
        )
        ...
        for attempt in range(self.max_retries):
            if attempt == 0:
                ...
                logger.warning(
                    f"[MANIM GEN] Previous errors to fix: {section_data.get('previous_errors')}"
                )
            ...
            if raw_completeness_errors:
                logger.warning(
                    f"[MANIM GEN] RAW Completeness check failed: {raw_completeness_errors}"
                )
                section_data["previous_errors"] = (
                    f"Code appears truncated or incomplete:\n"
            ...
            if code and "# Segment" in code:
                try:
                    ...
                    logger.warning(
                        f"[MANIM GEN] Hard Sync failed (proceeding with raw code): {e}"
                    )
            else:
                logger.debug(
                    "[MANIM GEN] Skipping Hard Sync: No '# Segment' markers found."
                )

            # Final Validation Phase
            # 1. Structural Validation (V2.6: Skip timing check if Hard Sync was applied)
            structural_errors = self.validate_code(
                code, section_data, skip_timing=has_sync
            )
            if structural_errors:
                logger.warning(
                    f"[MANIM GEN] Final validation failed: {structural_errors}"
                )
            ...
            if runtime_error:
                logger.warning(
                    f"[MANIM GEN] Runtime validation failed: {runtime_error}"
                )
    ...
    def _validate_runtime(
        self, code: str, section_data: Dict[str, Any]
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                return (
                    f"Manim Dry-Run Failed (RC={result.returncode}):\n{stderr[-1000:]}"
                )
    ...
    def validate_code(
        self, code: str, section_data: Dict[str, Any], skip_timing: bool = False
    ) -> List[str]:
        """
    ...
    def _check_structure(self, code: str) -> List[str]:
        ...
        if not re.search(r"class\s+\w+\(.*Scene\):", code):
            errors.append(
                "Missing Class definition inheriting from Scene (e.g. 'class MainScene(Scene):')"
            )
    ...
    def _check_forbidden_chars(self, code: str) -> List[str]:
        ...
        for char in forbidden:
            if char in code:
                errors.append(
                    f"Forbidden character detected: '{char}' (Replace with standard ASCII)"
                )
        ...
        try:
            ...
            if bad_char not in forbidden:  # Avoid duplicate error if already caught
                errors.append(
                    f"Non-ASCII character detected at position {e.start}: '{bad_char}'"
                )
    ...
    def _check_runtime_patterns(self, code: str) -> List[str]:
        ...
        always_redraw_matches = re.findall(
            r"always_redraw\s*\([^)]*(\w+)\.get_value", code
        )
        for tracker in always_redraw_matches:
            if not re.search(rf"{tracker}\s*=\s*ValueTracker", code):
                errors.append(
                    f"always_redraw references '{tracker}' but no ValueTracker with that name found"
                )
    ...
    def _check_completeness(self, code: str) -> List[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.endswith(":"):
                ...
                if not has_body:
                    errors.append(
                        f"Control structure at line {i + 1} has no body: '{stripped[:60]}...'"
                    )
        ...
        if last_logical:
            ...
            if (
                last_logical.isidentifier()
                and not keyword.iskeyword(last_logical)
                and last_logical not in ("self", "pass", "True", "False", "None")
            ):
                errors.append(
                    f"Code appears truncated at line {last_logical_idx}: bare identifier '{last_logical}'"
                )
            ...
            if last_logical.endswith(
                ...
                errors.append(
                    f"Code appears truncated at line {last_logical_idx}: ends with '{last_logical[-1]}'"
                )
        ...
        try:
            ...
            errors.append(
                f"Syntax error (possible truncation) at line {e.lineno}: {e.msg}"
            )
    ...
    def _check_syntax(self, code: str) -> List[str]:
        ...
        try:
            ...
            if undefined_names:
                ...
                logger.info(
                    f"[MANIM GEN] Validation note - potentially unknown names: {undefined_names[:5]}"
                )
        except Exception as e:
    ...
    def _check_placeholders(self, code: str) -> List[str]:
        ...
        for i, line in enumerate(code.split("\n"), 1):
            if dot_pattern.search(line):
                errors.append(
                    f"Line {i}: Dot() placeholder detected - use actual Manim objects instead"
                )

        return errors

    def _check_granular_timing(
        self, code: str, section_data: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        try:
            ...
            if not success:
                ...
                if fail_lines:
                    ...
                    errors.append(
                        "Manim timing validation failed (segments mismatched or cleanup violated)"
                    )
    ...
    def _enforce_timing(self, code: str, section_data: Dict[str, Any]) -> str:
        ...
        def process_block(lines_in_block, seg_idx):
                    ...
                    if not last_logical_line:
                        ...
                        if last_logical_line.endswith(("=", ",", "(", "+", "-", "*")):
                            lines_in_block.append(
                                f"{' ' * (len(line) - len(line.lstrip()))}# TRUNCATION DETECTED: Code ends mid-statement"
                            )
            ...
            if not has_fadeout:
                lines_in_block.append(
                    f"{indent}# V2.6: Clear screen for clean transition"
                )
                lines_in_block.append(
                    f"{indent}self.play(FadeOut(*self.mobjects), run_time={fadeout_duration})"
                )
            ...
            if deficit > 0.05:  # Use 0.05 as threshold to avoid tiny waits
                lines_in_block.append(
                    f"{indent}# Hard Sync: Injected wait to match audio ({actual_duration:.2f}s -> {target_duration:.2f}s)"
                )
                lines_in_block.append(f"{indent}self.wait({deficit:.3f})")
            elif deficit < -0.5:
                lines_in_block.append(
                    f"{indent}# Hard Sync WARNING: Animation exceeds audio by {abs(deficit):.2f}s"
                )
...
def build_manim_section_data(
    ...
    return {
        ...
        "visual_description": " ".join(
            _ensure_str(vb.get("description", "")) for vb in visual_beats
        ),
...
def integrate_manim_code_into_section(
    ...
    """
    DEPRECATED: This function no longer stores manim_code in presentation.json.

    V3 Pipeline generates Manim code fresh during render phase via
    _render_manim_segment_specs -> ManimCodeGenerator.generate_code().

    This function is kept for backward compatibility with older pipelines.
    It now only marks the section as having been processed.

    Args:
        section: The section dictionary
        manim_code: The generated Python code (ignored - not stored)

    Returns:
        Updated section (manim_code NOT added to JSON)
    """
    ...
    # Mark as having had code generation attempted
    if not render_spec.get("manim_scene_spec"):
        render_spec["manim_scene_spec"] = {}

    # NO LONGER storing manim_code in presentation.json
    # render_spec["manim_scene_spec"]["manim_code"] = manim_code
    # This was the bug - Python code was being stored in JSON

    render_spec["manim_scene_spec"]["code_generated"] = True
