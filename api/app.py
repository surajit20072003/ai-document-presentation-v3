        ...
        if "file" in request.files:
            ...
            if job_type == "document":
                ...
                if pipeline_version == "v3":
                    ...
                    print(f"[ROUTING DEBUG] Selected processor: process_markdown_job_v3 (V3 Three.js)")
                elif pipeline_version in ["v15_v2", "v15_v2_director"]:
...
def debug_job_status(job_id):
    ...
    try:
        ...
        try:
            ...
            elapsed_seconds = int((now_utc.replace(tzinfo=timezone.utc) - created_dt).total_seconds())
        except Exception:
            pass
        ...
        pipeline_info = {
            "version": params.get("pipeline_version", job.get("pipeline_version", "unknown")),
        ...
        if is_wip:
            ...
            if elapsed_seconds and elapsed_seconds > 3600:
                advisories.append(f"⚠️  Job has been running for {elapsed_seconds // 60} minutes. Consider checking for a stall.")
            elif elapsed_seconds:
                advisories.append(f"ℹ️  Job is actively processing. Running for {elapsed_seconds // 60}m {elapsed_seconds % 60}s.")
        ...
        pres_path = job_dir / "presentation.json"
        content_info = {"sections": 0, "segments": 0, "section_breakdown": {}, "renderers": {}}
        ...
        if pres_path.exists():
            ...
            try:
                ...
                for sec in sections:
                    ...
                    if renderer == "manim":
                        ...
                        expected_wan += 1
                    # count segments
                    content_info["segments"] += len(sec.get("narration", {}).get("segments", []))
            ...
            advisories.append("❌ presentation.json missing — content pipeline hasn't completed yet.")
        ...
        audio_files = []
        if audio_dir.exists():
            audio_files = [f.name for f in audio_dir.iterdir()
                           if f.is_file() and f.suffix in (".mp3", ".wav") and f.stat().st_size > 500]
        ...
        if len(audio_files) >= expected_audio and expected_audio > 0:
            score += 20
            advisories.append(f"✅ Audio: {len(audio_files)}/{expected_audio} files ready.")
        elif is_wip and current_phase in ("tts", "audio"):
            advisories.append(f"🔄 Audio: TTS in progress — {len(audio_files)}/{expected_audio} done so far.")
        elif current_phase not in ("tts", "audio") and len(audio_files) == 0 and is_wip:
            advisories.append(f"⏳ Audio: TTS hasn't started yet (current phase: {current_phase}).")
        else:
            advisories.append(f"⚠️  Audio incomplete: {len(audio_files)}/{expected_audio} files present.")

        # Videos
        video_dir   = job_dir / "videos"
        video_files = []
        if video_dir.exists():
            video_files = [f.name for f in video_dir.iterdir()
        ...
        if total_expected_video > 0:
            if len(video_files) >= total_expected_video:
                score += 15
                advisories.append(f"✅ Videos: {len(video_files)}/{total_expected_video} rendered.")
            elif is_wip and current_phase in ("manim", "video", "wan", "rendering", "visual_rendering"):
                advisories.append(f"🔄 Video rendering in progress — {len(video_files)}/{total_expected_video} done ({exp_manim} manim, {exp_wan} WAN expected).")
            else:
                advisories.append(f"⏳ Videos: {len(video_files)}/{total_expected_video} rendered — not started or pending.")
        else:
            advisories.append("ℹ️  No video rendering expected for this job (all static/none renderers).")
        ...
        avatar_files = []
        if avatar_dir.exists():
            avatar_files = [f.name for f in avatar_dir.iterdir()
        ...
        if len(avatar_files) >= expected_avatars and expected_avatars > 0:
            score += 15
            advisories.append(f"✅ Avatars: {len(avatar_files)}/{expected_avatars} generated.")
        elif len(avatar_files) > 0:
            score += int(15 * len(avatar_files) / max(expected_avatars, 1))
            if is_wip and current_phase == "avatar":
                advisories.append(f"🔄 Avatar generation in progress — {len(avatar_files)}/{expected_avatars} sections done.")
            else:
                advisories.append(f"⚠️  Avatars partial: {len(avatar_files)}/{expected_avatars} sections.")
        else:
            if is_wip and current_phase == "avatar":
                advisories.append(f"🔄 Avatar generation just started (0/{expected_avatars} done).")
            elif pipeline_info.get("skip_avatar"):
                advisories.append("ℹ️  Avatars skipped (skip_avatar=True).")
                score += 15
            else:
                advisories.append(f"⏳ Avatars: waiting — 0/{expected_avatars} generated yet.")
            ...
            try:
                ...
                for line in lines:
                    if "Total failures:" in line:
                        try:
                            validation_info["issues_count"] = int(line.split(":")[1].strip())
                        except Exception:
                            pass
                ...
                if validation_info["issues_count"] == 0:
                    ...
                    advisories.append(f"⚠️  Validation: {n} issue(s) flagged in v3_validation_report.txt.")
            except Exception as e:
        ...
        if analytics_path.exists():
            ...
            if dbg_path.exists():
                try:
                    ...
                    analytics_info["sections_processed"] = len(dbg) if isinstance(dbg, list) else "?"
                    score += 5
                except Exception:
                    pass
        ...
        if is_wip and elapsed_seconds:
            phase_timeout_minutes = {
                "tts": 30, "audio": 30,
                "manim": 60, "rendering": 90, "visual_rendering": 120,
                "wan": 120, "video": 120, "kie": 120,
                "avatar": 60
            }
        ...
        return jsonify({
            ...
            "elapsed_human": f"{elapsed_seconds // 3600}h {(elapsed_seconds % 3600) // 60}m {elapsed_seconds % 60}s" if elapsed_seconds else None,
            "error": job.get("error"),
            "source": source_info,
...
def process_document_job_v3(
    ...
    **kwargs,
    ...
    try:
        ...
        print(f"[V3 Document] Converted {file_ext}: {len(markdown_content)} chars, {page_count} pages")
...
def process_markdown_job_v3(
    ...
    **kwargs,
    ...
    """
    V3 pipeline job processor — Three.js-based pipeline.
